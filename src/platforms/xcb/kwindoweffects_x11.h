...
private:
    void slideWindowInternal(WId id, KWindowEffects::SlideFromLocation location, int offset);
    void enableBlurBehindInternal(WId window, bool enable, const QRegion &region);
    void enableBackgroundContrastInternal(WId window, bool enable, qreal contrast, qreal intensity, qreal saturation, const QRegion &region);
};
